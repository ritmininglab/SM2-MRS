...
%{
trunk_topwords = zeros(num_trunk,num_vocabulary);
trunk_wordcount = zeros(num_trunk,num_vocabulary);
for c=1:num_chain
    num_block=topic_assign(c,3);
    for t =4:num_block+3
        trunk = trunk_assign(c,t);
        narration_seq = narrs(c*field_narr-2,:);
        word = narration_seq(1,t);
        if word==0
            continue;
        end
        trunk_wordcount(trunk,word) = trunk_wordcount(trunk,word)+1;
    end
end
for i=1:num_trunk
    [value,idx] = sort(trunk_wordcount(i,:),'descend');
    trunk_topwords(i,:) = idx;
end
%}

csvwrite('topwords_topic.csv',topic_topwords);
%csvwrite('topwords_trunk.csv',trunk_topwords);

[popularwords, topwordidx] = sort(wordcounts,'descend');
csvwrite('popularwords.csv',[topwordidx,popularwords]);

%{
imgtopic = zeros(30,num_topic);
for c=1:num_chain
    img = topic_assign(c,1);
    num_block=topic_assign(c,3);
    for t =4:num_block+3
        topic = topic_assign(c,t);
        imgtopic(trunk,topic) = imgtopic(trunk,topic)+1;
    end
end
%}
