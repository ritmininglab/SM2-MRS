...
narrs1 = csvread('Data2/tmd3.csv');
narrs2 = csvread('Data2/tmd2.csv');
narrs = [narrs1;narrs2];
path1 = csvread('Data2/paths3.csv');
path2 = csvread('Data2/paths2.csv');
path = [path1;path2];
...
% for path
pos = 1;
response2 = zeros(nmap, field_pca);
Idx = zeros(num_chain, Tmax);
for c=1:num_chain
    num_block = MB(c,3);
    %%%%%%%%%%%%%%%%%%%%%% choose total_fields
    response2(pos:pos+num_block-lag_eye-1,:) = (path((c-1)*field_pca+1:c*field_pca, 4+lag_eye:3+num_block))';
    %response(pos:pos+num_block-lag_eye-1,:) = (Eye((c-1)*field_eye+2:(c-1)*field_eye+5, 4+lag_eye:3+num_block))';
    Idx(c,1:num_block) = pos:pos+num_block-lag_eye-1;
    pos = pos+num_block-lag_eye;
end
...
% store mixture mean
mix_param2 = cell(upperlimit,1); % mu
mix_S2 = cell(upperlimit,1); % intermediate variable to calculate sigma
mix_sigmainv2 = cell(upperlimit,1); % inverse sigma
mix_count2 = cell(upperlimit,1);
...
for p=1:num_topic
    for mix=1:1
        match3 = (match1==p); % .*(match2==mix);
        response_p = response2(match3==1,:);
        response_p = response_p';
        mix_count2{p,mix} = size(response_p,2); % number of events assigned to this pattern and mixture
        if mix_count2{p,mix}>0
            n = mix_count2{p,mix};
            xbar = mean(response_p,2);
            temp = response_p - repmat(xbar,1,mix_count2{p,mix});
            scattermat = temp * temp';
            mix_param2{p,mix} = kappa0/(kappa0+mix_count2{p,mix}) * mu0pca ...
                + mix_count2{p,mix}/(kappa0+mix_count2{p,mix}) * xbar; % horizontal 1-by-2 vector
            mix_S2{p,mix} = S0 + scattermat + kappa0*n / (kappa0+n) * (xbar-mu0pca)*(xbar-mu0pca)';
            mix_sigmainv2{p,mix} = inv(mix_S2{p,mix} / (n+df0-field_ar-1));
            % posterior var of mixmu is sigma/kappan, so if there are 10 obs, posterior var is really small already
        else
            mix_param2{p,mix} = mu0pca + sk*randn(field_pca, 1);
            mix_S2{p,mix} = S0;
            mix_sigmainv2{p,mix} = invS0;
        end
    end
end
...
pos = 1;
response3 = zeros(nmap, field_narr);
Idx = zeros(num_chain, Tmax);
for c=1:num_chain
    num_block = MB(c,3);
    %%%%%%%%%%%%%%%%%%%%%% choose total_fields
    response3(pos:pos+num_block-lag_eye-1,:) = (narrs((c-1)*field_narr+1:c*field_narr, 4+lag_eye:3+num_block))';
    %response(pos:pos+num_block-lag_eye-1,:) = (Eye((c-1)*field_eye+2:(c-1)*field_eye+5, 4+lag_eye:3+num_block))';
    Idx(c,1:num_block) = pos:pos+num_block-lag_eye-1;
    pos = pos+num_block-lag_eye;
end
...
% store mixture mean
mix_param3 = cell(upperlimit,1); % mu
mix_S3 = cell(upperlimit,1); % intermediate variable to calculate sigma
mix_sigmainv3 = cell(upperlimit,1); % inverse sigma
mix_count3 = cell(upperlimit,1);
...
for p=1:num_topic
    for mix=1:1
        match3 = (match1==p); % .*(match2==mix);
        response_p = response3(match3==1,:);
        response_p = response_p';
        mix_count3{p,mix} = size(response_p,2); % number of events assigned to this pattern and mixture
        if mix_count3{p,mix}>0
            n = mix_count3{p,mix};
            xbar = mean(response_p,2);
            temp = response_p - repmat(xbar,1,mix_count3{p,mix});
            scattermat = temp * temp';
            mix_param3{p,mix} = kappa0/(kappa0+mix_count3{p,mix}) * mu0narr ...
                + mix_count3{p,mix}/(kappa0+mix_count3{p,mix}) * xbar; % horizontal 1-by-2 vector
            mix_S3{p,mix} = S3 + scattermat + kappa0*n / (kappa0+n) * (xbar-mu0narr)*(xbar-mu0narr)';
            mix_sigmainv3{p,mix} = inv(mix_S3{p,mix} / (n+df0-field_narr-1));
            % posterior var of mixmu is sigma/kappan, so if there are 10 obs, posterior var is really small already
        else
            mix_param3{p,mix} = mu0narr + sk*randn(field_narr, 1);
            mix_S3{p,mix} = S3;
            mix_sigmainv3{p,mix} = invS3;
        end
    end
end
...
for iter=1:1%maxIter
    ...
    for doc = 1:num_chain
        ...
        for pos = 1:num_block
            ...
            r1 = zeros(num_topic, 1);
            for pattern = 1:num_topic
                mix = 1;
                r1(pattern,1) = 1000 * sqrt(det(mix_sigmainv{p,mix}))...
                    *exp(-0.5*(response_now - mix_param{p,mix})' * mix_sigmainv{p,mix} * (response_now - mix_param{p,mix}));
            end
            
            % Idx do not contain meta 3 columns, and do not contain lag_eye
            idx_you = Idx(doc,pos);
            response_now = response2(idx_you,:)';
            
            r2 = zeros(num_topic, 1);
            for pattern = 1:num_topic
                mix = 1;
                r2(pattern,1) = 1000 * sqrt(det(mix_sigmainv2{p,mix}))...
                    *exp(-0.5*(response_now - mix_param2{p,mix})' * mix_sigmainv2{p,mix} * (response_now - mix_param2{p,mix}));
            end
            
            % Idx do not contain meta 3 columns, and do not contain lag_eye
            idx_you = Idx(doc,pos);
            response_now = response3(idx_you,:)';
            
            r3 = zeros(num_topic, 1);
            for pattern = 1:num_topic
                mix = 1;
                r3(pattern,1) = 1000 * sqrt(det(mix_sigmainv3{p,mix}))...
                    *exp(-0.5*(response_now - mix_param3{p,mix})' * mix_sigmainv3{p,mix} * (response_now - mix_param3{p,mix}));
            end
...
%save('ldam.mat', 'tau','tau_prob','phi','phi_prob','B','mix_param','mix_S','mix_sigmainv');
